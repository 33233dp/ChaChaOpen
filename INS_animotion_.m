...
hold on;
% 绘制目标点（红点）
target_plot = plot(X_target, Y_target, 'ro', 'MarkerFaceColor', 'r', 'DisplayName', '目标点');
% 初始化小车位置（绿点）和朝向箭头
car_plot = plot(X(1), Y(1), 'go', 'MarkerFaceColor', 'g', 'DisplayName', '小车');
arrow_plot = quiver(X(1), Y(1), cos(theta(1)), sin(theta(1)), 0, 'Color', 'g', 'DisplayName', '小车朝向');
xlabel('x 位置 (m)');
ylabel('y 位置 (m)');
title('差速小车位置轨迹');
grid on;
...
% 固定坐标轴范围
xlim([min(0, X_target - 2) max(10, X_target + 2)]);
ylim([min(0, Y_target - 2) max(10, Y_target + 2)]);
...
for i = 1:num_steps
    ...
    % 更新小车位置（绿点）
    set(car_plot, 'XData', X(i), 'YData', Y(i));
    % 更新小车朝向箭头
    set(arrow_plot, 'XData', X(i), 'YData', Y(i), 'UData', cos(theta(i)), 'VData', sin(theta(i)));
    % 暂停一小段时间以显示动画效果
    pause(0.005);
    % 刷新图形
    drawnow;
