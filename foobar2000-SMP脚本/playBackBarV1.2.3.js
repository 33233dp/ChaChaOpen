...
// 评论功能 *****************************************************************************************************/
var g_font =  gdi.Font("Segoe UI", window.Height / 10, 1);
const g_text = "评论:";
var MetaDB_Comment;

function commentBox(gr){
    gr.FillSolidRect(btn_x, btn_y * 4, 200, 30,  0xFFCCCCCC);
    var NowPlay = fb.GetNowPlaying();

    //读取元数据中的评论
    MetaDB_Comment = fb.TitleFormat("%comment%").EvalWithMetadb(NowPlay);
    gr.GdiDrawText(g_text + MetaDB_Comment, g_font, 0xFF000000, btn_x, btn_y * 4, 200, 30);
}
...
function updateComment(){
    ...
    var input = utils.InputBox("请输入评论", "编辑评论", MetaDB_Comment);
